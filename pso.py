...
class Graph:
    ...
    def getCostPath(self, path):
        ...
        total_cost -= compute_avg_area(vanes)
        return total_cost
...
if __name__ == "__main__":
    ...
    graph = Graph(amount_vertices=numVanes)

    # This graph is in the folder "images" of the repository.
    for i in range(len(vanes)):
        for j in range(len(vanes)):
            if i != j:
                graph.addEdge(vanes[i], vanes[j], vanes[i].A + vanes[j].B)

    # creates a PSO instance
    pso = PSO(graph, iterations=1000, size_population=10, beta=0.85, alfa=0.85)
